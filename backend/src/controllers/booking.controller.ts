...
export async function getMyBookingsAsPro(req: Request, res: Response) {
  try {
    const bookings = await bookingService.getProBookings(req.user!.id)
    res.json({ bookings })
  } catch (err: any) {
    res.status(500).json({ error: err.message })
  }
...
export async function getBooking(req: Request, res: Response) {
  try {
    ...
    if (booking.owner_id !== req.user!.id && booking.pro_id !== req.user!.id) {
...
export async function rejectBooking(req: Request, res: Response) {
  ...
  try {
    const booking = await bookingService.reject(req.params.id, req.user!.id, parsed.data)
    res.json({ booking })
  } catch (err: any) {
    res.status(400).json({ error: err.message })
  }
...
export async function cancelBooking(req: Request, res: Response) {
  ...
  try {
    const booking = await bookingService.cancel(req.params.id, req.user!.id, parsed.data)
    res.json({ booking })
  } catch (err: any) {
    res.status(400).json({ error: err.message })
  }
...
export async function completeBooking(req: Request, res: Response) {
  try {
    const booking = await bookingService.complete(req.params.id, req.user!.id, req.body.pro_note)
    res.json({ booking })
  } catch (err: any) {
    res.status(400).json({ error: err.message })
  }
